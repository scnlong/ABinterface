...
The current main mechanism is the chain
\begin{equation}
A_v\uparrow
...
B_v\downarrow.
\end{equation}
This mechanism replaces the earlier, less transparent direct interface-SOC transition
\begin{equation}
A_c\uparrow \leftrightarrow B_v\downarrow,
\end{equation}
which bundled material change, spin flip, and band relaxation into a single phenomenological matrix element.  In the present implementation, spin flip is produced primarily by material-internal SOC mixing, while the interlayer transfer is spin-conserving.
...
The former impact-excitation branch has been removed from the main model.  Without explicit electron-electron Coulomb matrix elements and a controlled bath spectral density, impact excitation is too phenomenological for the present short-time chain mechanism and obscures pathway interpretation.
...
\end{equation}
For the default chain mechanism, spin-down CBM lies above spin-up CBM.  Therefore the internal sign convention is
\begin{equation}
...
\end{tabular}
\end{center}
The conduction-band mixing parameters are especially important because they allow the chain process
\begin{equation}
A_c\uparrow \rightarrow A_c\downarrow.
\end{equation}
...
\end{lstlisting}
The pathway-resolved plot is the most direct diagnostic for the proposed chain mechanism.
...
\texttt{dA0}, \texttt{dB0} & Optical dipole scales for materials $A$ and $B$. \\
\texttt{optical\_energy\_width} & Fallback optical resonance width. \\
...
\texttt{band\_overlap\_width} & Band-edge overlap width for optical and relaxation matrix elements. \\
\bottomrule
\end{longtable}
...
Parameter & Meaning \\
\midrule
\texttt{W\_downhill} & Post-pulse spin-conserving interlayer downhill transfer rate scale. \\
...
A physically consistent chain-mechanism run should show the following qualitative behavior:
\begin{enumerate}[leftmargin=2em]
  \item During the pulse, the laser produces $A_v\rightarrow A_c$ and $B_v\rightarrow B_c$ excitations.
  \item After the pulse, $A_c\uparrow$ can transfer population into $A_c\downarrow$ through material-internal SOC mixing.
  \item Spin-conserving interlayer hopping moves $A_c\downarrow$ population into $B_c\downarrow$.
  \item Intramaterial $c\rightarrow v$ relaxation transfers $B_c\downarrow$ population into $B_v\downarrow$.
